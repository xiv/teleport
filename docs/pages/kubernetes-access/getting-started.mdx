...
This Getting Started deploys Teleport on a Kubernetes cluster and provides Kubernetes access to users.  If you want to add Kubernetes access to an existing Teleport Cluster (cloud or on-prem) please see [Multiple Clusters](./guides/multiple-clusters/).
